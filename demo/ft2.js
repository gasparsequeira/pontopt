...
function Fasttracker()
{
  ...
  this.mixval=8.0;
   ...
   this.onRow = null;               // set this to function(position, row) { ... }
  this.onHighNote = null;          // set this to function(channelIndex, channelState) { ... }
  this.highNoteThreshold = 280;    // period threshold (smaller -> higher pitch). Tweak.
  this._lastRowPos = -1;           // internal guard to fire once per row
  this._lastRowRow = -1;
  
  this.highNoteMin = 58; // C-5 by default, adjust to taste
...
Fasttracker.prototype.advance = function(mod) {
  ...
  //console.log("mod position = " + mod.position + " mod.songlen = " + mod.songlen + " mod.repeat = " + mod.repeat);
...
Fasttracker.prototype.process_tick = function(mod) {
  ...
  for(var ch=0;ch<mod.channels;ch++) {
	...
	// piotr 
	 ...
	 // --- Row callbacks: run once per new row (after channels updated, BEFORE clearing flags) ---
  if (mod.flags & 2) { // new row
    if (mod._lastRowPos !== mod.position || mod._lastRowRow !== mod.row) {
      mod._lastRowPos = mod.position;
      mod._lastRowRow = mod.row;

      // prefer onRow (user property). fall back to _rowCallback if someone used that API.
      var rowCb = (typeof mod.onRow === 'function') ? mod.onRow :
                  (typeof mod._rowCallback === 'function') ? mod._rowCallback : null;
      if (rowCb) {
        try { rowCb(mod.position, mod.row); } catch (e) { console.error('onRow handler error', e); }
      }

      if (typeof mod.onHighNote === 'function') {
  for (var chn = 0; chn < mod.channels; chn++) {
    var cs = mod.channel[chn];
	
	//console.log("note = " + cs.note);
	
    if (cs && cs.noteon && cs.note >= mod.highNoteMin) {
      try { mod.onHighNote(chn, cs, cs.note); }
      catch (e) { console.error('onHighNote handler error', e); }
    }
  }
}
...
// piotr 
Fasttracker.prototype.onRow = function(callback) {
	console.log("Setting onROW");
  this._rowCallback = callback;
};
