...
const s_formId = '1FAIpQLSdBLR33-v3zw7NH0ee7YCt1SqPmKa6v3RJN5kl-zPBG0_TTSw';
const s_nameId = '871886387';    // "Name"
const s_websiteId = '1563119665'; // "Website"
const s_textId = '1494951178';   // "Text"
const s_pageId = '677122937';    // "Page"
const s_replyId = '1094706340';  // "Reply"
const s_sheetId = '1l3_-aYaQyP8Kez7j7Y4WJsv3r_416Bz8ZJ4aS4zDo_I';

// The values below are necessary for accurate timestamps, I've filled it in with EST as an example
const s_timezone = 0; // Portugal mainland = WET (UTC+0)
const s_daylightSavings = true; // Portugal observes EU DST (WEST, UTC+1)
// NOTE: EU DST is the LAST Sunday of March/October. This widget can't express "last",
// so 4 is used as an approximation (off by ~1h for at most one week/year). Adjust if you care.
const s_dstStart = ['March', 'Sunday', 4, 1]; // ~last Sunday of March, 01:00
const s_dstEnd = ['October', 'Sunday', 4, 1]; // ~last Sunday of October, 01:00
...
const s_maxLength = 400; // The max character length of a comment
...
const s_noCommentsText = 'no comments yet </3';
const s_closedCommentsText = 'comments are temporarily closed!';
const s_websiteText = '𓄲 site link '; // The links to websites left by users on their comments
        ...
        if (json.table.parsedNumHeaders > 0) { // Check if any comments exist in the sheet at all before continuing
            for (r = 0; r < json.table.rows.length; r++) {
                ...
                if (val1 == v_pagePath) { 
                    ...
                    // Locale-proof: column 0 is always the form timestamp, whatever its header
                    // is called (e.g. "Carimbo de data/hora" on a Portuguese account).
                    comment.Timestamp = json.table.rows[r].c[0].v;
